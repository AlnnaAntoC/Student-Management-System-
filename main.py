...
def addattendance():
    ...
    if request.method=="POST":
        ...
        print(attend,rollno,sub1,sub2,sub3,sub4,sub5,sub6)
        atte=Attendence(rollno=rollno,attendance=attend,sub1=sub1,sub2=sub2,sub3=sub3,sub4=sub4,sub5=sub5,sub6=sub6)
        db.session.add(atte)
        db.session.commit()
...
def search():
    if request.method=="POST":
        ...
        sub1=Attendence.query.filter_by(rollno=rollno).first()
        sub2=Attendence.query.filter_by(rollno=rollno).first()
        sub3=Attendence.query.filter_by(rollno=rollno).first()
        sub4=Attendence.query.filter_by(rollno=rollno).first()
        sub5=Attendence.query.filter_by(rollno=rollno).first()
        sub6=Attendence.query.filter_by(rollno=rollno).first()

        return render_template('search.html',bio=bio,attend=attend,sub1=sub1,sub2=sub2,sub3=sub3,sub4=sub4,sub5=sub5,sub6=sub6)
...
def addstudent():
    ...
    if request.method=="POST":
        ...
        query=Student(rollno=rollno,sname=sname,gender=gender,branch=branch,email=email,number=num,address=address)
